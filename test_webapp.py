...
def load_data_pattern(data_name):
    for i in range(1,6):
        print(df_main.loc[f'{data_name}-{i}'])
        data = df_main.loc[f'{data_name}-{i}']
        return data
    ...
    if st.button(f'Scan for {patterns}'):
        ...
        for filename in datafiles:  # loop for each file
            ...
            try:
                ...
                if last == 100:
                    crypto_dict[symbol][candlestick_patterns.get(patterns)] = 'bullish'  # set pattern value to use
                    ...
                    crypto_dict[symbol][candlestick_patterns.get(patterns)] = 'bearish'
                    print(crypto_dict[symbol], "\n")
                    functions.plot_with_bokeh.plot_candle_stick(crypto_dict[symbol]['crypto'])
                    st.bokeh_chart(functions.plot_with_bokeh.plot_candle_stick(crypto_dict[symbol]['crypto']))
